...
#         # rect = page.search_for(target_word, clip=page.rect)

#         # # Check if the target word was found on the page
#         # if rect and len(rect) > 0:
#         #     # Assuming the resized signature image is in PNG format
#         #     signature_pixmap = fitz.Pixmap(resized_signature_path)

#         #     # Calculate the position for the resized signature image
#         #     x = rect[0][0]
#         #     y = rect[0][1] - signature_pixmap.height
#         #     width = rect[0][2] - rect[0][0]
#         #     height = signature_pixmap.height

#         #     # Add the image to the page
#         #     page.insert_image((x, y, x + width, y + height), pixmap=signature_pixmap)

#         for inst in text_instances:
#             # Place the signature image below the found text instance
#             rect = fitz.Rect(inst[0], inst[3], inst[0] + 100, inst[3] + 50)
...
#             # Calculate the position for the resized signature image
#             # x = inst[0][0]
#             # y = inst[0][1] - signature_pixmap.height
#             # width = inst[0][2] - inst[0][0]
#             # height = signature_pixmap.height
